...
Created by: johnflux
Very Slightly Modified by: georgekuegler

    Note that this only works on Windows, since it requires the TrackIR.dll.
    The TrackIR software must be running, and use the 1:1 mapping in the TrackIR client if you want real values
...
class TrackIRDLL():
    ...
    def __init__(self, hWnd: Union[wintypes.HWND,int,str], trackir_profile_id: int =13302):  # Note that FreePIE uses id 13302, and 3750 is "Unity 64-bit"
        # 1302 = Dawn of Aces II
        # 3750 = Unity
        # 13302 = FreeSpace2
        """This function expects the TrackIR software to be installed. It will attempt to load the 64-bit or 32-bit dll.

           It raises an Exception if the TrackIR software is not installed.

           hWnd is a Window hWND, and is used by TrackIR to detect if your program has stopped. This is for legacy 
           programs that use the mouse cursor to look around the cockpit. TrackIR needed a way to ensure control was 
           relinquished back to the user for the mouse.  Called 'frame' in TKinter. e.g.
        ...
        # Test to see if Python is running in 32-bit or 64-bit
        # 32-bit versions of Python can only load 32-bit dlls
        if sys.maxsize > 2**32: 
            dllpath = path + "NPClient64.dll"
        else:
            dllpath = path + "NPClient.dll"
        ...
        # map the NP_StopCursor() and NP_StopCursor() function in the DLL.
        # These functions are used for legacy track IR setups and games where the mouse was used to control view.
        # These functions don't need to be called now. They're here just for reference completeness.
    ...
    def NP_RequestData(self, dataFields: int) -> int:
        """Call NP_RequestData in the Track IR dll to specify what data we want to receive.
           ...
           Roll 1              float +/- 16383 (representing +/- 180)
           Pitch 2             float +/- 16383 (representing +/- 180)
           Yaw 4               float +/- 16383 (representing +/- 180)
           X 16                float +/- 16383
           Y 32                float +/- 16383
           Z 64                float +/- 16383

           // x, y, z from raw imager position
           RawX 128            float 0..25600 (actual value is multiplied x 100 to pass two decimal places of precision)
           RawY 256            float 0..25600 (actual value is multiplied x 100 to pass two decimal places of precision)
           RawZ 512            float 0..25600 (actual value is multiplied x 100 to pass two decimal places of precision)

           // x, y, z deltas from raw imager position
           DeltaX 1024         float +/- 2560 (actual value is multiplied x 10 to pass two decimal places of precision)
           DeltaY 2048         float +/- 2560 (actual value is multiplied x 10 to pass two decimal places of precision)
           DeltaZ 4096         float +/- 2560 (actual value is multiplied x 10 to pass two decimal places of precision)

           // raw object position from imager
           SmoothX 8192        float 0..32766 (actual value is multiplied x 10 to pass one decimal place of precision)
           SmoothY 16384       float 0..32766 (actual value is multiplied x 10 to pass one decimal place of precision)
           SmoothZ 32768       float 0..32766 (actual value is multiplied x 10 to pass one decimal place of precision)
        """
